...
public class SimulatorServiceImpl {
	...
	public ArrayList<JSONObject> getAllFilteredRecordsWithConditions(
		...
		ArrayList<JSONObject> obj = Simulator
				.getAllFilteredRecordsWithConditions(resource,
						subResourceFilter, subResourceCond);

		return Simulator.getAllFilteredRecordsWithConditions(resource,
				subResourceFilter, subResourceCond);
	}

	// http://localhost:8080/WS_restful-simulator/rest/campaigns.json/30
	...
	// http://localhost:8080/WS_restful-simulator/rest/campaigns.json/30/filters-name&type
	...
	public JSONObject createObjectByID(JSONObject jsonlist,
		...
		System.out.println("--> OBJ aho " + obj);
		if (obj == null) {
			Response.ok(obj).status(404).build();
			return null;
		} else {
	...
	// http://localhost:8080/WS_restful-simulator/rest/campaigns.json/300/filters-name=pppp
	@PATCH
	@Path("/{id}/filters-{subResource: .+}")
	@Produces(MediaType.APPLICATION_JSON)
	public JSONObject updateMultipleFieldsByID(@PathParam("id") String id,
			...
			@PathParam("resource") String resource) {
		return Simulator.updateMultipleFieldsByID(id, subResource, resource);
	}

	// http://localhost:8080/WS_restful-simulator/rest/campaigns.json/300/
	@DELETE
	@Path("/{id}")
	@Produces(MediaType.APPLICATION_JSON)
